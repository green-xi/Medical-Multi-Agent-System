...
class TestCriticAgent:
    ...
    def test_fails_and_writes_replan_instruction_on_hallucination(self, state_with_docs):
        ...
        assert result["replan_instruction"] != ""
        assert "CriticAgent 第" in result["replan_instruction"] and "核查反馈" in result["replan_instruction"]
        # 代码里用 critic_attempt_count 记录已执行核查次数（进入后递增）
        assert result["critic_attempt_count"] == 1
        assert result["metrics"]["critic_pass"] is False

    def test_force_pass_when_retry_limit_reached(self, state_with_docs):
        """已达 MAX_CRITIC_ATTEMPTS=2 次，应强制通过并追加免责声明。"""
        # critic_attempt_count 是代码实际读取的字段名
        # 设为 MAX_CRITIC_ATTEMPTS=2，下次进入时递增为 3 > 2，触发强制通过
        state_with_docs["critic_attempt_count"] = 2
        with patch("app.agents.critic.get_llm") as mock_get:
            mock_get.return_value = MagicMock()
            result = CriticAgent(state_with_docs)
    ...
    def test_fails_when_answer_is_empty(self, base_state):
        ...
        assert result["critic_result"]["passed"] is True
        assert "没有查到" in result["generation"]

    def test_fails_when_answer_too_short(self, base_state):
        """过短答案同样返回友好提示并通过核查，避免死循环。"""
...
class TestResearchAgentLlmDirectPath:
    ...
    def test_llm_direct_overridden_when_replan_instruction_present(self, base_state):
        ...
        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._rag_fetch", return_value=[]), \
             patch("app.agents.research.rerank_documents", return_value=[]):
            mock_llm = MagicMock()
...
class TestResearchAgentReActLoop:
    ...
    def test_rag_search_action_adds_docs_to_state(self, base_state):
        ...
        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._rag_fetch", return_value=mock_docs), \
    ...
    def test_early_accept_when_rerank_score_above_threshold(self, base_state):
        ...
        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._rag_fetch", return_value=high_score_docs), \
    ...
    def test_force_accept_after_two_consecutive_expand_queries(self, base_state):
        ...
        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._rag_fetch", return_value=some_docs), \
    ...
    def test_tool_query_action_calls_tool_and_adds_doc(self, base_state):
        ...
        dummy_doc = Document(
            page_content="布洛芬是非甾体抗炎药，用于缓解疼痛和发热。",
            metadata={"rerank_score": 0.5},
        )
        # 预填文档到 base_state，跳过 iter=0 的无文档短路，
        # 使 iter=0 直接进入 LLM 决策（返回 tool_query）
        base_state["documents"] = [dummy_doc]

        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._run_tool", return_value=mock_tool_result), \
             patch("app.agents.research._rag_fetch", return_value=[dummy_doc]), \
             patch("app.agents.research.rerank_documents", return_value=[dummy_doc]):
            mock_llm = MagicMock()
            mock_llm.invoke.side_effect = side_effect
            mock_get.return_value = mock_llm
            result = ResearchAgent(base_state)
        ...
        # act_tool_query 将工具结果存入 state["tool_results"]，不写入 documents
        # documents 只存 RAG 检索文档，工具结果通过 tool_results 传给 _generate_answer
        assert result["tool_results"]["tool"] == "search_drug"
        assert "布洛芬" in result["tool_results"]["result"]
    ...
    def test_rag_think_log_scores_populated_on_llm_decision(self, base_state):
        ...
        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._rag_fetch", return_value=docs), \
        ...
        for entry in result["rag_think_log"]:
            ...
            assert isinstance(entry["scores"], dict), f"scores 必须是字典：{entry}"
            # expand_query 等无文档短路 action 不调用 LLM，scores 全为 None 是正常的
            # 只对 LLM 真正决策的 entry（accept / rag_search 等）断言 overall 有值
            if entry.get("action") not in ("expand_query", "rag_search"):
                assert entry["scores"].get("overall") is not None, \
                    f"LLM 决策 entry 缺少 overall 分数：{entry}"
...
class TestResearchAgentWithReplanInstruction:
    ...
    def test_injects_replan_instruction_into_think_prompt(self, base_state):
        ...
        # 预填文档：有文档 + replan_instruction 非空，iter=0 必定调用 LLM
        base_state["documents"] = [Document(
            page_content="维生素C与免疫功能相关背景资料。",
        ...
        with patch("app.agents.research.get_llm") as mock_get, \
             patch("app.agents.research._rag_fetch", return_value=base_state["documents"]), \
             patch("app.agents.research.rerank_documents", return_value=base_state["documents"]):
            mock_llm = MagicMock()
