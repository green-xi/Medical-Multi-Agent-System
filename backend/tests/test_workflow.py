...
class TestRouteAfterPlanner:
    ...
    def test_routes_to_research_after_initial_planning(self):
        """初始规划完成后（phase="init"），应去 research 执行。"""
        state = initialize_conversation_state()
        state["planner_eval"] = {
            "phase": "init",          # 显式标记：初始规划阶段
            "satisfied": False,
    ...
    def test_routes_to_critic_when_force_passed(self):
        """达到重规划上限强制放行时，phase="eval" + satisfied=True，应到 critic。"""
        state = initialize_conversation_state()
        state["planner_eval"] = {
            "phase": "eval",          # 显式标记：结果评估阶段
            "satisfied": True,
    ...
    def test_routes_to_research_when_waiting_for_execution(self):
        """phase="init" 表示初始规划刚完成，等待执行，应去 research。"""
        state = initialize_conversation_state()
        state["planner_eval"] = {
            "phase": "init",          # 显式标记：初始规划阶段
            "satisfied": False,
