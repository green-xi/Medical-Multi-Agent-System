...
class TestResearchAgentLlmDirectExtra:
    ...
    def test_llm_short_response_marks_success_false(self):
        ...
        # mock _generate_answer 直接返回短字符串，绕过 fallback 模板包装
        with patch("app.agents.research.get_llm", return_value=MagicMock()), \
             patch("app.agents.research.get_retriever", return_value=None), \
             patch("app.agents.research._generate_answer", return_value=("好", "系统提示")):
