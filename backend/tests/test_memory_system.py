...
TEST_DB = "tests/test_memory/memory_test.db"  # 仅保留常量，fixture 不再使用它


@pytest.fixture
def memory_service(tmp_path):
    """
    使用 pytest tmp_path 的独立 SQLite 文件，每个测试用例完全隔离。

    为什么改用 tmp_path：
    原实现用固定路径 TEST_DB，多个测试并行或上次测试文件残留时，
    Base.metadata.create_all() 会对已存在的表重建 ix_user_memory_session_id
    索引，抛出 "index already exists" 错误。
    tmp_path 由 pytest 为每个测试自动分配唯一临时目录，测试结束后自动清理。
    """
